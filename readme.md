...
| Indexer | BTC L1 与聪网的主网资产事实层 |
| STP / Transcend | BTC L1 与聪网之间的主网跨层资产控制、退出和惩罚协议 |
| SatoshiNet Core | 主网上线的交易、合约、应用和服务执行网络 |
| EVM Runtime | 已开发完成并上线测试网的 EVM 执行环境 |
| DKVS | 开发中的分布式键值基础设施方向 |
| Distributed L1 Indexer | 开发中的轻量化、可分布式验证的 L1 资产事实网络 |
...
STP：主网跨层控制、退出与惩罚
   ↓
SatoshiNet Core：主网交易、合约与应用执行
   ↓
EVM Runtime：测试网 EVM 执行环境
   ↓
...
| 开发者构建 | 接入 Indexer、钱包、STP、SatoshiNet Core、合约和社区 DEX / DAO | [开发者中心](build/readme.md) |
...
| SatoshiNet Core / Wallet / Explorer | Implemented · Mainnet | 聪网核心网络、钱包入口、浏览器和基础应用已经具备主网可用基础 |
| EVM Runtime | Testnet | EVM Runtime 已开发完成并上线测试网，继续验证 Solidity / ABI、UTXO 资产接口和开发者体验 |
| DKVS | In Development | 分布式键值基础设施正在开发中 |
| Distributed L1 Indexer | In Development | 分布式 L1 索引方案正在开发中，目标是降低运行索引器的存储与算力门槛 |
| VSN / Agent 可控钱包 / 长期治理 | Design / R&D | 继续进行设计、实验和验证，不应被表达为已完成生产能力 |
