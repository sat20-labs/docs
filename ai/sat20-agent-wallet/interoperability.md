# SAT20 Agent Wallet 互操作技能规范

> 本文面向需要自动操作 SAT20 Wallet 与 STP 通道的 AI Agent。目标是让 Agent 能够安全地理解、查询和执行资产在 BTC L1 与聪网之间的迁移。本文不是运行时代码，也不是某个 Agent 本地安装的 `SKILL.md`，而是 `SAT20 Agent Wallet` 的协议操作规范。
...
- 如果 adapter 返回 fee 不足，Agent 提示用户补充普通 BTC fee 资金，待确认后再重试；fee 输入仍由 adapter 内部选择。
...
- 一个 L1 UTXO 可能同时携带多种资产。当前 STP splicing-in 只 ascend 请求参数明确指定的资产；同一 UTXO 中未指定的 ORDX、Runes、BRC20 transfer 或 Ordinals NFT 都不进入聪网。Agent 将这些未指定资产排除在 L2 余额守恒预期之外。
- Agent 只提供资产名和金额，不要求用户指定 splicing-in 的资产输入。adapter 内部选币默认避开多资产 UTXO；如果必须使用，需要在预览里说明哪些资产会 ascend、哪些资产会被忽略。
...
| 字段 | 含义 |
| --- | --- |
| `feePolicy` | 可选 L1 费用策略；adapter 可忽略并使用内部估算 |
| `amt` | 初始通道金额，单位聪 |
| `memo` | 可选备注 |
...
Agent-facing adapter 不暴露底层资产输入或手续费输入参数。资产输入、BRC20 transfer inscription 和手续费输入都由 adapter 内部选择或构造。
...
Agent-facing adapter 不暴露 unlock 输入或 fee 输入参数。adapter 在通道内自动选择可用输入，并处理聪网手续费。
...
Agent-facing adapter 不暴露 lock 输入或 fee 输入参数。adapter 在用户聪网地址上自动选择资产输入和手续费输入；容量不足时返回 `INSUFFICIENT_CHANNEL_CAPACITY` 并建议 `lock-with-expand`。

## 状态追踪

Agent 持续查询 reservation 和链上状态，而不是只看发起接口是否返回成功。
...
7. `CS_CLOSE_FORCELY_*` 期间停止发起新的普通通道动作。
...
7. Agent 以聪网交易验证和资产守恒为准，不因为聪网找零小于 330 sats 就判断交易非法。
...
## 与 SAT20 Agent Wallet 的关系

`sat20-agent-wallet` 是本文规范的可安装形态。它把 Agent 能力拆成三个权限层次：

1. 只读查询：查询钱包、通道、reservation、余额、资产位置和安全快照。
2. 测试网操作：允许 testnet 自动 open、splicing、unlock、lock、lock-with-expand、splicing-out 和 punish drill。
3. 主网计划：mainnet 操作默认先生成计划和风险清单，执行前要求用户逐项确认。

这三个层次共享同一套状态模型，但权限和确认策略不同。这样既能让 Agent 真正操作 STP，又能避免在主网上出现不可逆误操作。
