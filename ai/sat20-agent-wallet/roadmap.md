# Agent 可控钱包与 SAT20 Agent Wallet 规划

> 本文从用户和 AI Agent 的共同视角定义 SAT20 PWA Wallet 与 SAT20 Agent Wallet 的目标能力。最终目标是让 Agent 能在用户授权下安全控制 BTC testnet/mainnet 与聪网资产流动，并能向用户解释为什么 STP 与聪网是比特币原生、安全可退出的扩展网络。
...
SAT20 PWA Wallet 不只是给人点击使用的钱包，也是 AI Agent 的安全执行层。
...
以下清单用于把 SAT20 Agent Wallet skill 从测试网联调能力推进到普通用户可安装、Agent 可长期操作的钱包能力。
...
| P1 | 测试网 fault API | 仅测试网构建提供保留和广播 Core Node 侧旧 commitment 的接口，主网构建不可用 |
| P1 | ORDX 通道演练 | 按 `bindingSat` 规则完成 ORDX splicing / unlock / lock 测试 |
| P2 | 视频素材采集工具 | 自动采集 PWA、L1 浏览器、L2 浏览器、indexer JSON、skill 输出截图 |
| P2 | Agent skill 分发 | 将 `sat20-agent-wallet` 作为可安装 skill 发布，并保持文档目录与用户安装目录一致 |
...
截至当前测试网演练，Agent 使用 `sat20-agent-wallet` skill 和本地 transcend adapter 已验证：
...
| 真实 punish drill | 已完成，包含 Core Node 侧旧 commitment 广播、client punish build、四笔 punish tx 广播和确认 |
