...
关于每个场景应检查哪些证据、缺少哪些数据时必须停止操作，以及 PWA adapter / indexer 还需要补齐哪些接口，见 [验证矩阵与数据缺口](verification-and-data-gaps.md)。
...
这些材料必须持久化保存。Agent 可以读取摘要和交易材料，但不直接接触私钥、助记词或未授权的 revocation secret。
...
client 要求 Core Node 保留当前 Core Node-local commitment 快照。该操作不广播交易，只保存演练材料。
...
在通道高度已经前进后，client 要求 Core Node 广播之前保留的 Core Node 侧旧 commitment。
...
2. Agent 调用 `stp.test_retain_server_commitment`，让 Core Node 保留当前 Core Node-side commitment。
...
5. Agent 调用 `stp.test_broadcast_retained_server_commitment`，触发 Core Node 广播 Core Node 侧旧 commitment。
