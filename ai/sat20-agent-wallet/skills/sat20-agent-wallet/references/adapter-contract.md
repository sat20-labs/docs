# SAT20 Wallet Adapter Contract

A SAT20 wallet adapter is any executable or callable service that maps JSON requests to SAT20 wallet and STP protocol actions.
...
```bash
$SAT20_CLIENT_CMD '{"op":"stp.status","chain":"testnet"}'
```
...
| Script | Use case |
| --- | --- |
| `scripts/stp_adapter.py` | Generic dispatcher from an Agent to `SAT20_ADAPTER_URL` or `SAT20_CLIENT_CMD` |
...
Legacy `STP_ADAPTER_URL` and `STP_CLIENT_CMD` remain supported by `scripts/stp_adapter.py` for existing testnet tooling.
...
- `amount`
- `to`
- optional `fee_policy` for BTC L1 sends
- optional `memo`
...
Rebuilds a channel from eligible channel-address evidence without creating a new SatoshiNet opening anchor. Use this when the L1 channel point changed but the corresponding plain sats were already anchored on SatoshiNet by an earlier opening anchor. This avoids duplicating SatoshiNet supply.

The Agent-facing request should not expose raw UTXO selection. The adapter selects an eligible plain-sat UTXO from the deterministic channel address and verifies that it is safe for no-anchor rebuild.
...
The Agent-facing request should describe the target asset and channel only. The adapter must discover eligible channel-address UTXOs from L1/L2 indexer evidence and reject ambiguous candidates instead of asking the Agent to choose inputs.
...
Agent-facing adapters must not expose asset-input or fee-input selection for `stp.splicing_in`. The request describes intent only: asset, amount, channel, chain, and optional L1 fee policy. The adapter selects suitable L1 asset inputs and BTC fee inputs internally.
...
Asks the Core Node to retain its current Core Node-local commitment snapshot. It does not broadcast anything. `commit_height` may be `0`.
...
Asks the Core Node to broadcast the retained old Core Node-side commitment after the channel has advanced to a later commit height. The expected next step is `stp.punish_build` / `stp.punish_broadcast` from the client wallet.
