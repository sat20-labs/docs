#!/usr/bin/env python3
"""Invoke a language-agnostic SAT20 wallet adapter.

The adapter can be configured as either:
- SAT20_CLIENT_CMD: a CLI command that accepts one JSON argument and returns JSON
- SAT20_ADAPTER_URL: an HTTP endpoint that accepts a JSON POST and returns JSON

Legacy STP_CLIENT_CMD and STP_ADAPTER_URL are still accepted.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Call a SAT20 wallet JSON adapter")
    ...
    adapter_url = os.environ.get("SAT20_ADAPTER_URL") or os.environ.get("STP_ADAPTER_URL")
    client_cmd = os.environ.get("SAT20_CLIENT_CMD") or os.environ.get("STP_CLIENT_CMD")
    ...
    if adapter_url:
        ...
        response = {
            "ok": False,
            "error_code": "SAT20_ADAPTER_NOT_CONFIGURED",
            "message": "Set SAT20_ADAPTER_URL or SAT20_CLIENT_CMD.",
        }
