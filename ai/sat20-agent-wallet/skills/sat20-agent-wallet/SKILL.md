---
name: sat20-agent-wallet
description: Use when operating or integrating with SAT20 Wallet, BTC L1 assets, SatoshiNet assets, STP channels, wallet/indexer status, safety snapshots, commitment export, punish coverage, force-close planning, testnet punish drills, and wallet/STP transaction tracking. Includes wallet creation/import/export, password changes, asset sends, channel open/reopen/rebuild/restore, expand, unlock, lock, lock-with-expand, splicing-in, splicing-out, and close. This skill is language-agnostic and works through any JSON-compatible SAT20 wallet adapter.
---

# SAT20 Agent Wallet

Use this skill to operate SAT20 Wallet and SatoshiNet assets through a language-agnostic SAT20 wallet adapter. STP channel operations are a core capability, but the skill is scoped to the broader SAT20 wallet surface: wallet lifecycle, direct asset sends, BTC L1 / SatoshiNet tracking, channel safety, and Agent-readable evidence.
...
The script uses `SAT20_ADAPTER_URL` for HTTP adapters, including a local SAT20 PWA Wallet adapter, or `SAT20_CLIENT_CMD` for CLI adapters. Legacy `STP_ADAPTER_URL` and `STP_CLIENT_CMD` are still accepted for existing testnet tooling.
...
```bash
SAT20_CLIENT_CMD="python3 scripts/stp_workspace_wallet_adapter.py" \
...
- Treat Core Node state as untrusted until signatures, chain, transaction outputs, assets, and commitment height are validated by the adapter.
...
1. Locate an adapter.
   - Prefer `SAT20_ADAPTER_URL` if the installed SAT20 PWA Wallet exposes a local HTTP JSON adapter.
   - Prefer `SAT20_CLIENT_CMD` if it wraps the SAT20 PWA Wallet bridge or a compatible third-party client, for example `stp-client --json`.
