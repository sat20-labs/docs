...
BUILD_DIR = ROOT / "build" / "voiceover-zh"
MANIFEST = ROOT / "one-minute-zh.json"
NARRATION = ROOT / "narration-zh.txt"
SILENT_VIDEO = ROOT / "build" / "sat20-agent-wallet-one-minute-zh-silent.mp4"
VOICE = "Ting-Ting"
RATE = "200"
...
def main() -> int:
    ...
    joined_audio = BUILD_DIR / "sat20-agent-wallet-one-minute-zh.m4a"
    run(
        [
            "ffmpeg",
            "-y",
    ...
    final_video = ROOT / "build" / "sat20-agent-wallet-one-minute-zh.mp4"
    run(
        [
            "ffmpeg",
            "-y",
