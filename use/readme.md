...
| 测试 Prediction / Agent 合约 | [Prediction 合约测试](prediction-contract.md) |
...
页面状态区分“协议/运行时已经存在”和“当前PWA是否有公开入口”。DAO、Launchpad、空投等存在历史或底层运行时，并不自动代表当前公开钱包已开放用户流程；进入操作前必须先看对应页面的可用性说明。
...
| 我想测试智能合约投注 | [Prediction 合约测试](prediction-contract.md) |
...
**页面状态：已实现 / 持续迭代（Implemented / Iterating）；具体功能以子页面可用性为准**
