...
> 本文记录 AI Agent 通过 `stp-core-node-client` skill 在 BTC testnet4 / 聪网测试环境中可以观察和验证的结果。本文只保留用户、Agent 和第三方客户端能通过钱包、adapter、L1/L2 浏览器或 indexer 复核的证据；不记录调试日志、内部函数、运行时修复过程或临时环境细节。
...
2. 如果旧 channel 数据已经删除或不可恢复，不要强行 `reopen`；应按普通 `stp.open` 新建通道，然后再 `stp.expand` / `stp.expand_all` 纳管通道地址上仍属于用户的资产。
...
1. BRC20 / Runes 在聪网上可以由 `0` sats 的 UTXO 携带，不要套用 BTC L1 dust 规则。
...
测试网演练的核心是让 core node 广播一个已经被撤销的旧 server commitment，然后让 client wallet 构造并广播 punish 包。Agent 只需要关注可验证证据：

| 阶段 | 证据 |
| --- | --- |
| 保留旧 server commitment | retained commit height `7` |
| 旧 server commitment | `151226853632d61d177b3279fcf99a7c144beaa3d018abd55f00f5b2adc24909` |
