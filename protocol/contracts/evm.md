...
1. `balanceOf(address, assetName)`：查询指定EVM地址对应合约地址当前可用的指定资产余额。
2. `transferAsset(assetName, to, amount, extraData)`：声明一次资产转移intent。该接口不直接花费UTXO，最终资产转移必须通过canonical `CONTRACT_RESULT`结算并由验证节点复核。

`receivedAsset(assetName)`不属于第一阶段接口。若后续需要表达“本次invoke随Call TX转入合约的指定资产数量”，应在协议和实现中单独定义；当前实现以Call TX输出、合约UTXO集合和Result TX验证作为资产输入与结算依据。
