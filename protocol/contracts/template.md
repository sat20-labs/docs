...
模版合约遵循[智能合约](./)的通用协议。本文只定义模版合约特有规则。
...
自动支付合约不来自通道合约迁移，而是第一阶段新增的协议内置模版，用于让部署者把一段区块区间内的固定或按高度变化的费用，自动转发给指定地址。
...
自动支付合约用于按区块高度向一个指定地址支付费用，模版名称为：
...
1. 接收地址。
2. 支付资产名称，可以是聪或任意聪网资产。
3. 支付模式，第一阶段支持固定金额和线性金额。
4. 基础支付金额。
5. 线性模式下的每区块增量。
6. 可选结束高度。
...
2. 合约激活高度由部署后首次满足资金条件的区块确定，实际支付从激活高度的下一个区块开始。
3. 如果设置了结束高度，合约必须有足够支付整个剩余区间的支付资产和trigger result gas，才能进入active状态。
4. 如果没有设置结束高度，合约只要求有足够支付下一个区块的支付资产和trigger result gas；余额不足支付下一个区块时进入funding状态。
5. 如果支付资产本身就是gas资产，runtime会在同一种物理资产中区分业务支付余额和合约trigger gas余额。

支付规则：

1. 每个区块结算时，runtime检查是否到达`NextPayHeight`。
2. 满足条件时，出块节点自动生成canonical `CONTRACT_RESULT`，把本区块应付金额转给接收地址。
3. 固定模式每个支付区块使用相同金额。
4. 线性模式按`base + step * offset`计算，其中`offset`从第一次支付区块开始计数。
5. 合约支付失败不会由外部交易补充执行；余额不足时只改变为funding状态，等待后续funding。

关闭规则：

1. 只有deployer可以关闭自动支付合约。
2. close时，合约管理的剩余支付资产和剩余gas余额退回deployer。
3. 合约地址上不属于合约管理余额的其他资产，继续按通用合约framework的剩余资产处理规则执行。
